        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="searchProducts" placeholder="Search products or vendor..."
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Origin Filter</label>
                ...
                <div>
                    ...
                    <select id="sortProducts"
                        ...
                        <option value="usr">Vendor Entries</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div id="creatorInfo" class="mt-1 text-xs"></div>
                </div>
            </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" id="addPackageNameBtn"
                                class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">Add</button>
                        </div>
...
<script>
        ...
        fetch(`${BASE_URL}admin/fetch/manageProductPackages.php?action=createPackageName`, {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    input.value = '';
                    const newPackage = { id: data.id, package_name: packageName };
    ...
    function renderProductsTable(products) {
        ...
        tbody.innerHTML = products.map(product => {
            ...
            return '<tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="showProductModal(\'' + product.id + '\')">' +
                '<td class="px-4 py-3 whitespace-nowrap"><div class="flex items-center"><div class="flex-shrink-0 h-10 w-10 rounded-lg overflow-hidden bg-gray-100"><img src="' + mainImage + '" alt="' + escapeHtml(product.title) + '" class="w-full h-full object-cover"></div><div class="ml-4"><div class="text-sm font-medium text-secondary max-w-xs hover:text-primary"><span class="hidden sm:block truncate">' + escapeHtml(product.title) + '</span><span class="sm:hidden break-words">' + escapeHtml(product.title) + '</span></div><div class="text-xs text-gray-500 hidden sm:block">ID: ' + product.id + '</div></div></div></td>' +
                '<td class="px-4 py-3 text-center"><span class="text-sm text-gray-900">' + escapeHtml(product.category_name || 'Uncategorized') + '</span></td>' +
                '<td class="px-4 py-3 text-center">' + creator + '</td>' +
                '<td class="px-4 py-3 text-center">' + statusBadge + '</td>' +
                '<td class="px-4 py-3 text-center">' + featuredBadge + '</td>' +
                '</tr>';
        }).join('');
    }
    ...
    function renderProductsCards(products) {
        ...
        container.innerHTML = products.map(product => {
            ...
            return '<div class="p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer" onclick="showProductModal(\'' + product.id + '\')">' +
                ...
                '<div class="flex-shrink-0 h-12 w-12 rounded-lg overflow-hidden bg-gray-100"><img src="' + mainImage + '" alt="' + escapeHtml(product.title) + '" class="w-full h-full object-cover"></div>' +
                '<div class="flex-1 min-w-0">' +
                '<div class="mb-1"><div class="flex items-center gap-1 mb-1">' + statusBadge + featuredBadge + '</div>' +
                '<h4 class="text-sm font-medium text-secondary hover:text-primary pr-2 break-words">' + escapeHtml(product.title) + '</h4></div>' +
                '<div class="text-xs text-gray-500 mb-2">' + escapeHtml(product.category_name || 'Uncategorized') + '</div>' +
                '<div class="mb-2">' + creator + '</div>' +
                '<div class="text-xs text-gray-600 mb-2 line-clamp-2">' + escapeHtml((product.description || 'No description available')) + '</div>' +
                packageNamesHtml +
                '<div class="hidden sm:block"><div class="text-xs text-gray-500 mt-2">ID: ' + product.id + '</div></div>' +
                '</div>' +
                '</div>' +
                '</div>';
        }).join('');
    ...
    function getStatusBadge(status) {
        switch (status) {
            ...
            case 'draft': return '<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Draft</span>';
    ...
    function showProductModal(productId) {
        ...
        if (productId) {
            ...
            creatorInfo.innerHTML = '<span class="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-medium bg-sky-100 text-sky-800">Admin</span>';
            modal.classList.remove('hidden');
            initSortable();
        }
    }
    ...
    function resetProductForm() {
        ...
        document.getElementById('created-by-user-id').value = '';
        document.getElementById('created-by-user-type').value = '';
    ...
    function populateProductForm(prod) {
        ...
        document.getElementById('productStatus').value = prod.status || 'draft';
        document.getElementById('productFeatured').checked = !!(prod.featured == 1 || prod.featured === true);
        if (prod.package_names && prod.package_names.length > 0) {
            selectedPackageNames = prod.package_names.map(pkg => ({ id: pkg.id, name: pkg.package_name }));
            renderSelectedPackageNames();
            updatePackageNameSearchPlaceholder();
        }
    ...
    function saveProduct() {
        ...
        const category = categoryVal === '' ? null : categoryVal;
    ...
    function cropAndSaveImage() {
        ...
        canvas.toBlob(blob => {
            ...
            fetch(`${BASE_URL}admin/fetch/manageProducts.php?action=uploadImage`, { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
                    hideLoading();
                    if (data.success) {
