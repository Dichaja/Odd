...
function submitReview(PDO $pdo, string $currentUser, string $username)
{
    ...
    try {
        ...
        $existingStmt = $pdo->prepare("SELECT id FROM product_reviews WHERE user_id = ? AND product_id = ?");
        $existingStmt->execute([$currentUser, $productId]);
        ...
        if ($existingStmt->fetch()) {
            ...
            ");
            $updateStmt->execute([$rating, $comment, $now, $currentUser, $productId]);
            $message = 'Review updated successfully';
        } else {
            // Insert new review
            $insertStmt = $pdo->prepare("
                INSERT INTO product_reviews (id, product_id, user_id, username, rating, comment, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $insertStmt->execute([$reviewId, $productId, $currentUser, $username, $rating, $comment, $now, $now]);
            $message = 'Review submitted successfully';

            // Create notification for admin about new review
            try {
                $notificationService = new NotificationService($pdo);
                $adminMessage = "New product review: {$username} rated \"{$product['title']}\" {$rating}/5 stars.";
                ...
                $notificationService->create(
                    ...
                    'New Product Review',
                    $recipients,
                    "/product-details.php?id={$productId}#reviews",
                    'normal',
                    $currentUser
                );
