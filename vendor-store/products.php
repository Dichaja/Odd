...
<div x-data="productsPage()" x-init="init()" class="space-y-6">
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
        <div class="hidden sm:flex items-center gap-2 mb-5">
            <button @click="switchTab('inStore')" :class="activeTab==='inStore'?'tab-active':'tab-idle'" class="tab-btn">My Store Products</button>
            <button @click="switchTab('myProducts')" :class="activeTab==='myProducts'?'tab-active':'tab-idle'" class="tab-btn">My Created Products</button>
        </div>
        ...
        <form @submit.prevent="applySearch()" class="flex flex-col sm:flex-row gap-3 mb-6">
            <div class="relative flex-1">
                <input x-model="search" type="text" placeholder="Search by name or category" class="w-full pl-3 pr-12 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary focus:border-transparent">
                <button type="submit" class="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 w-8 rounded-md grid place-items-center bg-gray-900 hover:bg-black text-white">
                    <i data-lucide="search" class="w-4 h-4"></i>
                </button>
            </div>
            <div class="flex gap-2">
                <button type="button" @click="clearSearch()" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition flex items-center gap-2">
                    <i data-lucide="x" class="w-4 h-4"></i>
                    Clear
                </button>
                <div class="relative" x-show="activeTab==='inStore'">
                    <button type="button" @click="toggleAddMenu()" class="px-4 py-2 bg-user-primary text-white rounded-lg hover:bg-user-primary/90 transition flex items-center gap-2">
                        <i data-lucide="plus" class="w-5 h-5"></i>
                        Add Product
                    </button>
                    <div x-show="addMenuOpen" @click.outside="addMenuOpen=false" class="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-10">
                        <button @click="openAddExisting()" class="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-2">
                            <i data-lucide="boxes" class="w-4 h-4"></i>
                            Select From Existing
                        </button>
                        <button @click="openCreateNew()" class="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-2">
                ...
                <div class="relative" x-show="activeTab==='myProducts'">
                    <button type="button" @click="openCreateNew()" class="px-4 py-2 bg-user-primary text-white rounded-lg hover:bg-user-primary/90 transition flex items-center gap-2">
        ...
        <div x-show="!loading && filteredActiveList().length>0" id="productsGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <template x-for="p in pagedActiveList()" :key="p.store_product_id ?? p.id">
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-200 flex flex-col h-full">
                    <div class="relative">
                        <img :src="p._image || placeholderImg" @error="$event.target.src=placeholderImg" :alt="p.name || p.title" class="w-full h-44 object-cover rounded-t-xl bg-gray-100">
                        <template x-if="activeTab==='inStore'">
                            <div class="absolute top-2 right-2 flex gap-2">
                                <button @click="openPricingList(p)" class="bg-white px-3 py-1.5 rounded-full shadow-md hover:bg-gray-50 transition text-gray-700 text-sm font-medium flex items-center gap-1">
                        ...
                        <template x-if="activeTab==='myProducts'">
                            <div class="absolute top-2 right-2 flex gap-2">
                                <button @click="openPricingForMy(p)" class="bg-white px-3 py-1.5 rounded-full shadow-md hover:bg-gray-50 transition text-gray-700 text-sm font-medium flex items-center gap-1">
                                    <i data-lucide="tags" class="w-4 h-4"></i>
                                    Edit Pricing
                                </button>
                                <button @click="openEditMyProduct(p)" :disabled="!p.editable" class="px-3 py-1.5 rounded-full shadow-md transition text-sm font-medium flex items-center gap-1" :class="p.editable?'bg-white hover:bg-gray-50 text-gray-700':'bg-gray-200 text-gray-400 cursor-not-allowed'">
                        ...
                        <div class="flex-1">
                            ...
                            <template x-if="activeTab==='myProducts'">
                                <div class="mt-2 flex items-center justify-between">
                                    <p class="text-xs" :class="(p.status||'').toLowerCase()==='draft'?'text-yellow-700':'text-green-700'" x-text="(p.status||'').toUpperCase() || 'DRAFT'"></p>
                                    <button x-show="(p.status||'').toLowerCase()==='draft'" @click="confirmDeleteMyDraft(p)" class="text-xs text-red-600 hover:underline">Delete Draft</button>
                                </div>
                            </template>
                        </div>
                        <template x-if="activeTab==='inStore'">
                            <div class="mt-auto">
                                <div class="flex flex-wrap gap-1.5 mb-3">
                                    ...
                                    <template x-for="pr in (p.pricing||[])" :key="(pr.pricing_id||pr.package_mapping_id)+'-'+(pr.price_category||'')">
                                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-medium" :class="chipColor(pr.price_category)">
                                ...
                                <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                                    <span class="text-xs text-gray-500" x-text="(p.pricing?.length || 0)+' pricing option'+((p.pricing?.length||0)===1?'':'s')"></span>
                                    <button @click="confirmDeleteFromStore(p)" class="p-2 rounded-full text-red-600 hover:bg-red-50 transition">
        ...
        <div x-show="!loading && totalPages>1" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div class="text-sm text-gray-700">
                <span x-text="'Showing '+(offset()+1)+' to '+Math.min(offset()+limit, filteredActiveList().length)+' of '+filteredActiveList().length"></span>
            </div>
            <div class="flex items-center gap-1">
                <button @click="goto(page-1)" x-bind:disabled="page===1" class="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-40 disabled:pointer-events-none">Previous</button>
                <template x-for="n in pageNumbers()" :key="n">
                    <button @click="goto(n)" class="px-3 py-2 text-sm border rounded-lg transition" :class="n===page ? 'bg-user-primary text-white border-user-primary' : 'border-gray-300 hover:bg-gray-50'">
                        <span x-text="n"></span>
                    </button>
                </template>
                <button @click="goto(page+1)" x-bind:disabled="page===totalPages" class="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-40 disabled:pointer-events-none">Next</button>
            </div>
        </div>
    </div>
                ...
                <div class="relative bg-white shadow-xl w-full max-w-2xl modal-panel overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="openCreateNewFromSelect()" class="px-3 py-1.5 text-sm bg-user-primary text-white rounded-lg hover:bg-user-primary/90">Create New</button>
                    ...
                    <div class="modal-scroll p-5 space-y-4">
                        <div class="relative">
                            ...
                            <input x-model="selectSearch" type="text" placeholder="Search products to add" class="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary focus:border-transparent">
                        </div>
                        <div class="divide-y">
                            ...
                            <template x-for="p in selectableProducts()" :key="p.id">
                                <div class="flex items-center justify-between py-3">
                                    ...
                                    <button @click="beginAddPricing(p)" class="px-3 py-1.5 text-sm bg-user-primary text-white rounded-lg hover:bg-user-primary/90 transition flex items-center gap-1">
                        ...
                        <div class="grid md:grid-cols-3 gap-6">
                            <div class="md:col-span-2 space-y-4">
                                <div>
                                    ...
                                    <input x-model="newProduct.title" type="text" placeholder="Product title" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary">
                                </div>
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Description</label>
                                    <textarea x-model="newProduct.description" rows="4" placeholder="Product description" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary"></textarea>
                                </div>
                                <div>
                                    ...
                                    <div class="custom-select" x-data="{open:false}" @keydown.escape.stop="open=false">
                                        <button type="button" class="custom-select__btn" @click="open=!open">
                                            <span x-text="selectedPackages.length ? selectedPackages.map(p=>p.package_name).join(', ') : 'Select package names'"></span>
                                            <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                        </button>
                                        <div x-show="open" @click.outside="open=false" class="custom-select__panel">
                                            <div class="p-2 sticky top-0 bg-white border-b">
                                                <input x-model="pkgSearch" type="text" placeholder="Filter..." class="w-full px-2 py-1 border border-gray-300 rounded">
                                            </div>
                                            ...
                                            <template x-for="m in filteredPackages()" :key="m.id">
                                                <div class="custom-select__opt" @mousedown.prevent="toggleSelectPackage(m)">
                                                    <span x-text="m.package_name"></span>
                                                    <i :data-lucide="isSelectedPackage(m.id)?'check-square':'square'" class="w-4 h-4"></i>
                                                </div>
                                            </template>
                                            <div class="border-t">
                                                <button class="w-full custom-select__opt" @mousedown.prevent="createNewPackageName()">
                                                    <span x-text="pkgSearch ? ('Add &quot;' + pkgSearch + '&quot;') : 'Add new package'"></span>
                                                    <i data-lucide="plus" class="w-4 h-4"></i>
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        <template x-for="p in selectedPackages" :key="p.id">
                                            <span class="custom-select__tag">
                                                ...
                                                <button class="ml-1 text-gray-500 hover:text-red-600" @click="removeSelectedPackage(p.id)">
                            ...
                            <div class="space-y-3">
                                <div class="w-full bg-gray-50 rounded-lg border border-gray-200 p-2">
                                    <div class="grid grid-cols-3 gap-2">
                                        <template x-for="(img, idx) in newProduct.previews" :key="idx">
                                            <div class="relative">
                                                <img :src="img" class="w-full aspect-video object-cover rounded-md bg-gray-100">
                                                <button type="button" @click="removeNewImage(idx)" class="absolute top-1 right-1 p-1 rounded-full bg-white shadow text-red-600">
                                                    <i data-lucide="x" class="w-4 h-4"></i>
                                                </button>
                                            </div>
                                        </template>
                                        <template x-if="newProduct.previews.length===0">
                                            <div class="col-span-3 text-center text-gray-400 text-sm py-6">No images</div>
                                        </template>
                                    </div>
                                </div>
                                <input type="file" accept="image/*" multiple class="hidden" x-ref="fileInput" @change="handleNewImages">
                                <button type="button" @click="$refs.fileInput.click()" class="w-full px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">
                                    Upload Images
                                </button>
                                ...
                                <p class="text-xs text-red-600" x-show="newProduct.temp_images.length===0">At least one image is required</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-5 border-t flex justify-end gap-2">
                        <button @click="closeNewProduct()" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">Cancel</button>
                        <button @click="submitNewProduct()" class="px-4 py-2 rounded-lg bg-user-primary text-white hover:bg-user-primary/90">Create</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <template x-teleport="body">
        <div x-show="modals.editMyProduct" x-transition.opacity class="fixed inset-0 z-[1000] m-0 p-0">
            ...
            <div class="fixed inset-0 flex items-center justify-center p-3">
                <div class="relative bg-white shadow-2xl w-full max-w-3xl modal-panel overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b">
                        <h3 class="text-lg font-semibold text-secondary" x-text="editProduct.id ? 'Edit Product' : 'Edit Product'"></h3>
                    ...
                    <div class="modal-scroll p-5 space-y-6">
                        <div class="grid md:grid-cols-3 gap-6">
                            <div class="md:col-span-2 space-y-4">
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Title</label>
                                    <input x-model="editProduct.title" type="text" placeholder="Product title" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary">
                                </div>
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Description</label>
                                    <textarea x-model="editProduct.description" rows="4" placeholder="Product description" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary"></textarea>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <div class="w-full aspect-video bg-gray-100 rounded-lg overflow-hidden grid place-items-center border border-gray-200">
                                    <img x-show="editProduct.preview" :src="editProduct.preview" class="w-full h-full object-cover" alt="">
                                    <div x-show="!editProduct.preview" class="text-gray-400 text-sm">No image</div>
                                </div>
                                <input type="file" accept="image/*" class="hidden" x-ref="fileInputEdit" @change="handleEditImage">
                                <button type="button" @click="$refs.fileInputEdit.click()" class="w-full px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50" :disabled="!editProduct.editable">
                                    Upload Image
                                </button>
                                ...
                                <p class="text-xs text-gray-500" x-show="!editProduct.editable">Only draft products can be edited</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-5 border-t flex justify-end gap-2">
                        <button @click="closeEditMyProduct()" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">Cancel</button>
                        <button @click="submitEditMyProduct()" class="px-4 py-2 rounded-lg bg-user-primary text-white hover:bg-user-primary/90" :disabled="!editProduct.editable">Save</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="relative bg-white shadow-xl w-full max-w-3xl modal-panel overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b">
                        <div class="flex items-center gap-3">
                            <img :src="pricingProduct?._image || placeholderImg" class="w-10 h-10 rounded-md object-cover bg-gray-100" alt="">
                            <div>
                                <h3 class="text-lg font-semibold text-secondary" x-text="pricingProduct?.name ? 'Manage Pricing - '+pricingProduct.name : 'Manage Pricing'"></h3>
                    ...
                    <div class="modal-scroll p-5 space-y-4">
                        <div class="flex items-center justify-between">
                            <p x-show="isDraftProduct()" class="text-sm text-yellow-800 bg-yellow-50 px-3 py-1.5 rounded">Product awaiting publishing from admin.</p>
                            <button @click="openStepper('new')" :disabled="!canAddPricing()" class="px-3 py-2 rounded-lg transition flex items-center gap-2" :class="canAddPricing() ? 'bg-gray-900 text-white hover:bg-black' : 'bg-gray-200 text-gray-500 cursor-not-allowed'">
                                <i data-lucide="plus" class="w-4 h-4"></i>
                                Add Pricing
                            </button>
                        </div>
                                ...
                                <template x-for="(pr,idx) in pricingList" :key="idx">
                                    <div class="grid grid-cols-12 gap-2 px-4 py-3 border-t items-center bg-white">
                                        <div class="col-span-3">
                                            <div class="text-sm" x-text="(pr.package_size||'-')+' '+(pr.si_unit||labelForUnit(pr.si_unit_id)||'')"></div>
                                        </div>
                                        <div class="col-span-3">
                                            <div class="text-sm font-medium" x-text="pr.package_name || labelForPackage(pr.package_mapping_id)"></div>
                                        </div>
                                        <div class="col-span-2">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" :class="chipColor(pr.price_category)" x-text="(pr.price_category||'').toUpperCase()"></span>
                                        </div>
                                        <div class="col-span-2">
                                            <div class="text-sm font-semibold" x-text="'UGX '+formatNumber(pr.price)"></div>
                                            <div class="text-[11px] text-gray-500" x-text="formatCommissionMini(pr)"></div>
                                        </div>
                                        <div class="col-span-1 text-center">
                                            <div class="text-xs text-gray-600" x-text="pr.delivery_capacity ? pr.delivery_capacity : '-'"></div>
                                        </div>
                                        <div class="col-span-1 text-right">
                                            <button @click="openStepper('edit', idx)" class="p-2 rounded hover:bg-gray-50">
                                                <i data-lucide="pen-line" class="w-4 h-4"></i>
                                            </button>
                                        </div>
                                    </div>
                    ...
                    <div class="p-5 border-t flex justify-end gap-2">
                        <button @click="closePricingList()" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">Close</button>
                        <button @click="savePricingChanges()" class="px-4 py-2 rounded-lg bg-user-primary text-white hover:bg-user-primary/90" x-text="pricingProduct?.store_product_id ? 'Save Changes' : 'Add To Store'"></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="modals.stepper" x-transition.opacity class="fixed inset-0 z-[1000] m-0 p-0">
            ...
            <div class="fixed inset-0 flex items-center justify-center p-3">
                <div class="relative bg-white shadow-2xl w-full max-w-4xl modal-panel overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b">
                        <div class="flex items-center gap-3">
                            <img :src="stepperProductImage || pricingProduct?._image || placeholderImg" class="w-10 h-10 rounded-md object-cover bg-gray-100" alt="">
                            <div>
                                <h3 class="text-lg font-semibold text-secondary" x-text="stepper.mode==='new' ? 'Add Pricing' : 'Edit Pricing'"></h3>
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div x-show="stepper.step===1" class="space-y-2">
                                    ...
                                    <div class="custom-select" x-data="{open:false}">
                                        ...
                                        <div x-show="open" @click.outside="open=false" class="custom-select__panel">
                                            <div class="p-2 sticky top-0 bg-white border-b">
                                                <input x-model="stepper.packageQuery" type="text" placeholder="Filter..." class="w-full px-2 py-1 border border-gray-300 rounded">
                                            </div>
                                            <template x-if="availablePackages.length===0">
                                                <div class="p-3 text-center text-gray-500">No packages</div>
                                            </template>
                                            <template x-for="m in availablePackages.filter(x=>x.package_name.toLowerCase().includes((stepper.packageQuery||'').toLowerCase()))" :key="m.id">
                                                <div class="custom-select__opt" @mousedown.prevent="selectPackage(m)">
                                                    <span x-text="m.package_name"></span>
                                                    <i :data-lucide="stepper.package_mapping_id===m.id?'check':'plus'" class="w-4 h-4"></i>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                <div x-show="stepper.step===2" class="space-y-4">
                                    <div>
                                        ...
                                        <div class="custom-select" x-data="{open:false}">
                                            ...
                                            <div x-show="open" @click.outside="open=false" class="custom-select__panel">
                                                <div class="p-2 sticky top-0 bg-white border-b">
                                                    <input x-model="stepper.unitQuery" type="text" placeholder="Filter..." class="w-full px-2 py-1 border border-gray-300 rounded">
                                                </div>
                                                <template x-for="u in availableUnits.filter(x=>x.si_unit.toLowerCase().includes((stepper.unitQuery||'').toLowerCase()))" :key="u.id">
                                                    <div class="custom-select__opt" @mousedown.prevent="selectUnit(u)">
                                                        <span x-text="u.si_unit"></span>
                                                        <i :data-lucide="stepper.si_unit_id===u.id?'check':'plus'" class="w-4 h-4"></i>
                                                    </div>
                                                </template>
                                            </div>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <input x-model="stepper.package_size" type="text" inputmode="text" placeholder="Enter size" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary" :class="errors.size ? 'border-red-500 ring-2 ring-red-300' : ''" required>
                                    </div>
                                </div>
                                <div x-show="stepper.step===3" class="space-y-4">
                                    <div>
                                        ...
                                        <div class="custom-select" x-data="{open:false}">
                                            <button type="button" class="custom-select__btn" @click="open=!open">
                                                <span x-text="stepper.price_category ? stepper.price_category.toUpperCase() : 'Select category'"></span>
                                                <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                            </button>
                                            <div x-show="open" @click.outside="open=false" class="custom-select__panel">
                                                <div class="custom-select__opt" @mousedown.prevent="stepper.price_category='retail';open=false">Retail</div>
                                                <div class="custom-select__opt" @mousedown.prevent="stepper.price_category='wholesale';open=false">Wholesale</div>
                                                <div class="custom-select__opt" @mousedown.prevent="stepper.price_category='factory';open=false">Factory</div>
                                            </div>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <input x-model="stepper.price" type="number" min="0" step="any" placeholder="Enter price" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary" :class="errors.price ? 'border-red-500 ring-2 ring-red-300' : ''">
                                    </div>
                                </div>
                                <div x-show="stepper.step===4" class="space-y-2">
                                    <label class="text-sm font-medium text-gray-700" x-text="stepper.price_category==='retail' ? 'Max Capacity' : (stepper.price_category ? 'Min Capacity' : 'Capacity')"></label>
                                    <input x-model="stepper.delivery_capacity" type="number" min="0" step="1" placeholder="Enter capacity" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary" :class="errors.capacity ? 'border-red-500 ring-2 ring-red-300' : ''">
                                </div>
                                <div x-show="stepper.step===5" class="space-y-4">
                                    <div>
                                        ...
                                        <div class="custom-select" x-data="{open:false}">
                                            <button type="button" class="custom-select__btn" @click="open=!open">
                                                <span x-text="stepper.commission_type==='flat' ? 'Flat' : 'Percentage'"></span>
                                                <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                            </button>
                                            <div x-show="open" @click.outside="open=false" class="custom-select__panel">
                                                <div class="custom-select__opt" @mousedown.prevent="stepper.commission_type='percentage';onCommissionTypeChange();open=false">Percentage</div>
                                                <div class="custom-select__opt" @mousedown.prevent="stepper.commission_type='flat';onCommissionTypeChange();open=false">Flat</div>
                                            </div>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <input x-model="stepper.commission_value" type="number" min="0" step="any" placeholder="Enter commission" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-user-primary" :class="errors.commissionValue ? 'border-red-500 ring-2 ring-red-300' : ''">
                                        <p class="text-xs mt-1" :class="errors.commissionValue ? 'text-red-600' : 'text-gray-500'" x-text="commissionHint()"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="hidden md:block">
                                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                                    <div class="relative">
                                        <img :src="stepperProductImage || pricingProduct?._image || placeholderImg" class="w-full h-40 object-cover bg-gray-100" alt="">
                                        <div class="absolute top-2 right-2">
                                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-medium" :class="chipColor(stepper.price_category || 'retail')">
                                                <i data-lucide="tags" class="w-3 h-3"></i>
                                                <span x-text="(stepper.package_size?stepper.package_size:'')+(stepper.si_unit?' '+stepper.si_unit:'') + (stepper.package_name?(' • '+stepper.package_name):'')"></span>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="p-4 space-y-2">
                                        <h4 class="font-semibold text-gray-900" x-text="pricingProduct?.name || 'Product'"></h4>
                                        <p class="text-sm text-gray-600" x-text="pricingProduct?.category_name || ''"></p>
                                        <div class="mt-2 flex flex-wrap gap-1.5">
                                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-medium" :class="chipColor(stepper.price_category || '')">
                                                <i data-lucide="tag" class="w-3 h-3"></i>
                                                <span x-text="(stepper.price_category||'').toUpperCase() || 'CATEGORY'"></span>
                                            </span>
                                        </div>
                                        <div class="flex items-baseline justify-between mt-2">
                                            <div class="text-sm text-gray-500">Price</div>
                                            <div class="text-lg font-bold" x-text="'UGX '+formatNumber(stepper.price || 0)"></div>
                                        </div>
                                        <div class="flex items-baseline justify-between">
                                            <div class="text-sm text-gray-500">Commission</div>
                                            <div class="text-sm font-medium" x-text="stepper.commission_type==='flat' ? ('UGX '+formatNumber(stepper.commission_value||0)) : ((stepper.commission_value||0)+'%')"></div>
                                        </div>
                                        <div class="flex items-baseline justify-between">
                                            <div class="text-sm text-gray-500">Capacity</div>
                                            <div class="text-sm font-medium" x-text="stepper.delivery_capacity || '-'"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="p-5 border-t flex items-center justify-between">
                        <button @click="prevStep()" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 flex items-center gap-2" :disabled="stepper.step===1">
                            <i data-lucide="chevron-left" class="w-4 h-4"></i>
                            Back
                        </button>
                        <div class="flex items-center gap-2">
                            <button x-show="stepper.step<5" @click="nextStep()" class="px-4 py-2 rounded-lg bg-gray-900 text-white hover:bg-black flex items-center gap-2">
                                Next
                                <i data-lucide="chevron-right" class="w-4 h-4"></i>
                            </button>
                            <button x-show="stepper.step===5" @click="commitStepper()" class="px-4 py-2 rounded-lg bg-user-primary text-white hover:bg-user-primary/90 flex items-center gap-2">
                ...
                <div class="relative bg-white shadow-xl w-full max-w-md modal-panel overflow-hidden">
                    <div class="p-6">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <h3 class="text-lg font-semibold text-secondary" x-text="deleteContextType==='store' ? 'Confirm Remove From Store' : 'Confirm Delete Draft'"></h3>
                        </div>
                        <p class="text-gray-600 mb-2" x-text="deleteContextType==='store' ? 'Remove this product from your store?' : 'Delete this draft product permanently?'"></p>
                        <p class="text-sm font-medium text-gray-900" x-text="deleteContext?.name || deleteContext?.title"></p>
                    </div>
                    <div class="p-6 border-t flex justify-end gap-2">
                        <button @click="modals.deleteConfirm=false" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">Cancel</button>
                        <button @click="performDelete()" class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 flex items-center gap-2">
                            <i data-lucide="trash-2" class="w-4 h-4"></i>
                            Delete
                        </button>
                    </div>
            ...
            <div class="fixed inset-0 flex items-center justify-center p-3">
                <div class="relative bg-white shadow-xl w-full max-w-md modal-panel overflow-hidden">
                    ...
                    <div class="p-6 border-t flex justify-end">
                        <button @click="modals.notice=false" class="px-4 py-2 rounded-lg bg-gray-900 text-white hover:bg-black">OK</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    function productsPage() {
        return {
            ...
            async reloadAll() {
                this.loading = true;
                await Promise.all([this.fetchStoreProducts(), this.fetchMyProducts()]);
                this.loading = false;
            ...
            totalPages() {
                return Math.max(1, Math.ceil(this.filteredActiveList().length / this.limit));
            },
            pageNumbers() {
            ...
            pagedActiveList() {
                return this.filteredActiveList().slice(this.offset(), this.offset() + this.limit);
            },
            goto(n) {
            ...
            async fetchStoreProducts() {
                try {
                    ...
                    if (j.success && j.products) {
                        this.productsInStore = await Promise.all(j.products.filter(p => p.store_product_id).map(async p => {
            ...
            async openPricingList(p) {
                ...
                if (!this.pricingProduct._image) {
                    this.pricingProduct._image = await this.getImage(p.id);
                }
                await this.loadMetaForProduct(p.id);
            ...
            async createNewPackageName() {
                ...
                try {
                    ...
                    if (j.success && j.id) {
                        ...
                        if (!this.isSelectedPackage(j.id)) {
                            this.selectedPackages.push(obj);
                        }
                        this.pkgSearch = '';
            ...
            async openStepper(mode, idx = null) {
                ...
                if (!this.pricingProduct?._image) {
                    this.pricingProduct._image = await this.getImage(this.pricingProduct.id);
                }
            ...
            commitStepper() {
                ...
                if (this.errors.commissionValue) {
                    return;
                }
