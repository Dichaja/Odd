...
<div class="min-h-screen bg-gray-50" x-data="storeVisitsPage()" x-init="init()" x-on:keydown.escape="closeAnyModal()">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full">
                    <div class="hidden sm:flex flex-wrap gap-2">
                        <button :class="dateBtnClass('daily')" @click="setPeriod('daily')" class="flex-1 sm:flex-none px-4 py-2 rounded-lg border transition-colors text-sm">
                            <span class="inline-flex items-center gap-2"><i data-lucide="calendar-days" class="w-4 h-4"></i>Daily</span>
                        </button>
                        <button :class="dateBtnClass('weekly')" @click="setPeriod('weekly')" class="flex-1 sm:flex-none px-4 py-2 rounded-lg border transition-colors text-sm">
                            <span class="inline-flex items-center gap-2"><i data-lucide="calendar" class="w-4 h-4"></i>Weekly</span>
                        </button>
                        <button :class="dateBtnClass('monthly')" @click="setPeriod('monthly')" class="flex-1 sm:flex-none px-4 py-2 rounded-lg border transition-colors text-sm">
                            <span class="inline-flex items-center gap-2"><i data-lucide="calendar-range" class="w-4 h-4"></i>Monthly</span>
                        </button>
                        <button :class="dateBtnClass('yearly')" @click="setPeriod('yearly')" class="flex-1 sm:flex-none px-4 py-2 rounded-lg border transition-colors text-sm">
                            <span class="inline-flex items-center gap-2"><i data-lucide="calendar-clock" class="w-4 h-4"></i>Yearly</span>
                        </button>
                    </div>
                    <div class="sm:hidden w-full">
                        <label class="sr-only">Period</label>
                        <select x-model="filters.period" @change="setPeriod(filters.period)" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900">
                    ...
                    <button @click="openCalendar()" class="px-4 py-2 rounded-lg border text-sm bg-white text-gray-900 hover:bg-gray-50 inline-flex items-center gap-2">
                        <i data-lucide="calendar" class="w-4 h-4"></i>
                        <span x-text="rangeLabel"></span>
                    </button>
                    <button @click="applyCurrentRange()" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors text-sm">Apply</button>
                </div>
            </div>
                ...
                <div class="rounded-xl p-4 border bg-yellow-50 border-yellow-200">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xl font-bold text-yellow-900 truncate" x-text="stats.pending.toLocaleString()"></p>
                        </div>
                ...
                <div class="rounded-xl p-4 border bg-blue-50 border-blue-200">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xl font-bold text-blue-900 truncate" x-text="stats.confirmed.toLocaleString()"></p>
                        </div>
                ...
                <div class="rounded-xl p-4 border bg-green-50 border-green-200">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xl font-bold text-green-900 truncate" x-text="stats.completed.toLocaleString()"></p>
                        </div>
                ...
                <div class="rounded-xl p-4 border bg-red-50 border-red-200">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xl font-bold text-red-900 truncate" x-text="stats.cancelled.toLocaleString()"></p>
                        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 mb-8">
            <div class="p-4 sm:p-6 border-b border-gray-100">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div class="relative">
                            <input type="text" placeholder="Search requests..." x-model.debounce.400ms="filters.search" @input="page=1;loadRequests()" class="w-full sm:w-auto pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white text-gray-900 placeholder-gray-400">
                            <i data-lucide="search" class="w-4 h-4 absolute left-3 top-2.5 text-gray-400"></i>
                        </div>
                        <select x-model="filters.status" @change="page=1;loadRequests()" class="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900">
                        ...
                        <button @click="clearFilters()" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 border border-gray-300 rounded-lg hover:bg-gray-50">Clear Filters</button>
                    </div>
                </div>
            </div>
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="w-full min-w-[760px]">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Customer Details</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Product</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Visit Date</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Quantity</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Total Value</th>
                        ...
                        <template x-for="r in requests" :key="r.id">
                            <tr class="hover:bg-gray-50 transition-colors cursor-pointer" @click="openDetails(r.id)">
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="font-medium text-gray-900 text-sm" x-text="r.first_name+' '+r.last_name"></div>
                                    <div class="text-xs text-gray-500" x-show="r.status==='confirmed'" x-text="r.email"></div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="text-sm font-medium text-gray-900 truncate" x-text="r.product_title"></div>
                                    <div class="text-xs text-gray-500" x-text="capitalize(r.price_category)+' - UGX '+formatCurrency(r.price)"></div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800" x-text="r.quantity"></span>
                                </td>
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    <span class="text-sm font-bold text-green-600" x-text="'UGX '+formatCurrency(r.total_value)"></span>
                                </td>
            ...
            <div class="lg:hidden">
                ...
                <template x-for="r in requests" :key="'m-'+r.id">
                    <div class="p-4 border-t border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer" @click="openDetails(r.id)">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-1">
                                    <h4 class="text-sm font-medium text-gray-900 truncate" x-text="r.first_name+' '+r.last_name"></h4>
                                ...
                                <div class="text-xs text-gray-500 mb-2" x-text="'UGX '+formatCurrency(r.price)+' per unit'"></div>
                                <div class="flex items-center justify-between">
                                ...
                                <div class="mt-1 text-xs text-gray-500" x-show="r.status==='confirmed'" x-text="r.email"></div>
                            </div>
                        </div>
                    </div>
                </template>
                <div class="p-4 border-t border-gray-100 flex items-center justify-between">
                    <div class="text-sm text-gray-600">Showing <span x-text="showingFrom"></span>-<span x-text="showingTo"></span> of <span x-text="total"></span></div>
                    <div class="flex items-center gap-2">
                        <button @click="prev()" :disabled="page===1" class="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50">Previous</button>
                        <span class="px-3 py-1 text-sm text-gray-600" x-text="'Page '+page+' of '+Math.max(1,totalPages)"></span>
                        <button @click="next()" :disabled="page===totalPages || totalPages===0" class="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50">Next</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-cloak x-show="calendar.open" x-transition.opacity class="fixed inset-0 z-[65]">
        ...
        <div class="relative w-full max-w-lg mx-auto top-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-4 border-b flex items-center justify-between">
                <div class="text-sm font-semibold" x-text="calendarTitle"></div>
                <button class="p-2 rounded hover:bg-gray-100" @click="closeCalendar()"><i data-lucide="x" class="w-5 h-5"></i></button>
            </div>
            <div class="p-4 max-h-[80vh] overflow-y-auto">
                <template x-if="filters.period==='weekly' || filters.period==='daily'">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                                <button class="p-2 rounded border hover:bg-gray-50" @click="navMonth(-1)"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                                <button class="p-2 rounded border hover:bg-gray-50" @click="navMonth(1)"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                            </div>
                            <div class="flex items-center gap-2">
                                <select x-model.number="calendar.viewMonth" @change="syncCalFromControls()" class="px-2 py-1 border rounded bg-white text-gray-900 text-sm">
                                    <template x-for="(m, i) in months" :key="'m'+i">
                                        <option :value="i" x-text="m"></option>
                                    </template>
                                </select>
                                <input type="number" x-model.number="calendar.viewYear" @change="syncCalFromControls()" class="w-24 px-2 py-1 border rounded bg-white text-gray-900 text-sm">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            <template x-for="cell in monthGrid" :key="cell.key">
                                <button class="w-full aspect-square rounded border text-sm relative" :class="[
                                    ...
                                    <span x-show="isSameDate(cell.date, filters.start) && filters.period==='daily'" class="absolute right-1 top-1 inline-block w-2 h-2 rounded-full bg-primary"></span>
                                </button>
                            </template>
                        </div>
                        <div class="mt-3 flex items-center justify-end gap-2">
                            <button class="px-3 py-2 border rounded hover:bg-gray-50 text-sm" @click="goToday()">Today</button>
                            <button class="px-3 py-2 bg-primary text-white rounded text-sm" @click="applyFromCalendar()">Use Range</button>
                        </div>
                    </div>
                </template>
                ...
                <template x-if="filters.period==='monthly'">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2">
                                <button class="p-2 rounded border hover:bg-gray-50" @click="calendar.viewYear--; syncCalFromControls()"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                                <div class="text-sm font-medium" x-text="calendar.viewYear"></div>
                                <button class="p-2 rounded border hover:bg-gray-50" @click="calendar.viewYear++; syncCalFromControls()"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                            </div>
                            <input type="number" x-model.number="calendar.viewYear" @change="syncCalFromControls()" class="w-28 px-2 py-1 border rounded bg-white text-gray-900 text-sm">
                        </div>
                        <div class="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            <template x-for="(m, i) in months" :key="'mm'+i">
                                <button class="px-3 py-4 border rounded text-sm bg-white hover:bg-gray-50" :class="isSelectedMonth(i) ? 'ring-2 ring-primary/60 ring-offset-1' : ''" @click="selectMonth(i)">
                                    <span x-text="m"></span>
                                </button>
                            </template>
                        </div>
                        <div class="mt-4 text-right">
                            <button class="px-3 py-2 border rounded hover:bg-gray-50 text-sm" @click="goThisMonth()">Current Month</button>
                        </div>
                    </div>
                </template>
                ...
                <template x-if="filters.period==='yearly'">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <button class="p-2 rounded border hover:bg-gray-50" @click="calendar.viewYear--"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                            <input type="number" x-model.number="calendar.viewYear" class="w-28 px-2 py-1 border rounded bg-white text-gray-900 text-sm text-center">
                            <button class="p-2 rounded border hover:bg-gray-50" @click="calendar.viewYear++"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                        </div>
                        <div class="text-center">
                            <button class="px-4 py-2 bg-primary text-white rounded" @click="selectYear(calendar.viewYear)">Use Year</button>
                        </div>
                    </div>
                </template>
            </div>
    ...
    <div x-cloak x-show="modals.details" x-transition.opacity class="fixed inset-0 z-50">
        ...
        <div class="relative w-full max-w-6xl mx-auto my-4 sm:my-8 bg-white rounded-xl shadow-lg max-h-[92vh] overflow-hidden">
            <div class="p-4 sm:p-6 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-blue-50 to-blue-100">
                <div>
                ...
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2 px-3 py-1 rounded-full border" :class="statusPill.bg">
                        ...
                        <span class="text-xs font-medium" :class="statusPill.text" x-text="capitalize(requestDetails.status)"></span>
                    </div>
                    <button @click="closeDetails()" class="text-gray-500 hover:text-gray-700 p-2 rounded-lg hover:bg-white/60">
                        <i data-lucide="x" class="w-5 h-5"></i>
                    </button>
                </div>
            </div>
            <div class="flex flex-col lg:flex-row max-h-[calc(92vh-88px)] overflow-hidden">
                <aside class="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-gray-200 bg-gray-50 overflow-y-auto max-h-[50vh] lg:max-h-none">
                    <div class="p-4">
                                ...
                                <template x-if="requestDetails.status==='confirmed'">
                                    <div class="bg-white rounded-lg p-4 border border-gray-200">
                                        ...
                                        <div class="space-y-2 text-sm">
                                            <div class="flex justify-between"><span class="text-gray-600">Name:</span><span class="font-medium" x-text="requestDetails.first_name+' '+requestDetails.last_name"></span></div>
                                            <div class="flex justify-between"><span class="text-gray-600">Email:</span><span class="font-medium" x-text="requestDetails.email"></span></div>
                                            <div class="flex justify-between"><span class="text-gray-600">Phone:</span><span class="font-medium" x-text="requestDetails.phone"></span></div>
                                            <template x-if="requestDetails.alt_contact"><div class="flex justify-between"><span class="text-gray-600">Alt Contact:</span><span class="font-medium" x-text="requestDetails.alt_contact"></span></div></template>
                                            <template x-if="requestDetails.alt_email"><div class="flex justify-between"><span class="text-gray-600">Alt Email:</span><span class="font-medium" x-text="requestDetails.alt_email"></span></div></template>
                                        </div>
                                    </div>
                                </template>
                                <div class="bg-white rounded-lg p-4 border border-gray-200">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <span class="font-medium" x-text="formatDate(requestDetails.visit_date)"></span>
                                                <template x-if="requestDetails.status==='confirmed'">
                                                    <button @click="openRescheduleCalendar(requestDetails.id, requestDetails.visit_date)" class="text-blue-600 hover:text-blue-800 text-xs p-1 rounded hover:bg-blue-50"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
                                                </template>
                                            </div>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Status:</span>
                                            <span class="font-medium" :class="visitInfo(requestDetails).color" x-text="visitInfo(requestDetails).text"></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-white rounded-lg p-4 border border-gray-200">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between"><span class="text-gray-600">Product:</span><span class="font-medium text-right" x-text="requestDetails.product_title"></span></div>
                                        <div class="flex justify-between"><span class="text-gray-600">Package:</span><span class="font-medium text-right" x-text="requestDetails.package_size+' '+requestDetails.si_unit+' '+requestDetails.package_name"></span></div>
                                        <div class="flex justify-between"><span class="text-gray-600">Category:</span><span class="font-medium text-right" x-text="capitalize(requestDetails.price_category)"></span></div>
                                        <div class="flex justify-between"><span class="text-gray-600">Quantity:</span><span class="font-medium" x-text="requestDetails.quantity"></span></div>
                                        <div class="flex justify-between"><span class="text-gray-600">Unit Price:</span><span class="font-medium" x-text="'UGX '+formatCurrency(requestDetails.price)"></span></div>
                                        <div class="flex justify-between border-t pt-2"><span class="text-gray-900 font-medium">Total Value:</span><span class="font-bold text-green-600" x-text="'UGX '+formatCurrency(requestDetails.total_value)"></span></div>
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="space-y-6">
                            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                    <i data-lucide="box" class="w-5 h-5 text-blue-600"></i><span>Product Information</span>
                                </h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div><p class="text-sm font-medium text-gray-600">Product</p><p class="font-medium" x-text="requestDetails.product_title"></p></div>
                                    <div><p class="text-sm font-medium text-gray-600">Category</p><p class="font-medium" x-text="capitalize(requestDetails.price_category)"></p></div>
                                    <div><p class="text-sm font-medium text-gray-600">Package</p><p class="font-medium" x-text="requestDetails.package_size+' '+requestDetails.si_unit+' '+requestDetails.package_name"></p></div>
                                    <div><p class="text-sm font-medium text-gray-600">Quantity</p><p class="font-medium" x-text="requestDetails.quantity"></p></div>
                                </div>
                            </div>
                            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                    <i data-lucide="calendar-check" class="w-5 h-5 text-green-600"></i><span>Visit Details</span>
                                </h4>
                                <div class="space-y-4">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2 mt-1">
                                                <p class="font-medium" x-text="formatDate(requestDetails.visit_date)"></p>
                                                <template x-if="requestDetails.status==='confirmed'">
                                                    <button @click="openRescheduleCalendar(requestDetails.id, requestDetails.visit_date)" class="text-blue-600 hover:text-blue-800 text-sm p-1 rounded hover:bg-blue-50"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
                                                </template>
                                            </div>
                                            <p class="text-sm mt-1" :class="visitInfo(requestDetails).color" x-text="visitInfo(requestDetails).text"></p>
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium text-gray-600">Request Made</p>
                                            <p class="font-medium mt-1" x-text="formatDate(requestDetails.created_at)"></p>
                                            <p class="text-sm text-gray-500" x-text="formatTime(requestDetails.created_at)"></p>
                                        </div>
                                    </div>
                                    <template x-if="requestDetails.notes">
                                        <div class="border-t pt-4">
                                            <p class="text-sm font-medium text-gray-600">Customer Notes</p>
                                            <p class="mt-1 text-gray-900 bg-gray-50 p-3 rounded border" x-text="requestDetails.notes"></p>
                                        </div>
                                    </template>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                        <i data-lucide="phone" class="w-5 h-5 text-purple-600"></i><span>Communication</span>
                                    </h4>
                                    <template x-if="requestDetails.status!=='confirmed'">
                                        <div class="p-4 border border-dashed border-gray-300 rounded-lg text-sm text-gray-600">Confirm this request to view customer communication options.</div>
                                    </template>
                                    <template x-if="requestDetails.status==='confirmed'">
                                        <div class="grid grid-cols-1 gap-3">
                                            <button @click="callCustomer(requestDetails.phone)" class="flex items-center p-3 bg-green-50 rounded-lg border border-green-200 hover:bg-green-100 transition-colors">
                                                <div class="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center mr-3"><i data-lucide="phone" class="w-5 h-5 text-white"></i></div>
                                                <div class="flex-1 text-left"><p class="font-medium text-green-900">Call Customer</p><p class="text-sm text-green-700" x-text="requestDetails.phone"></p></div>
                                                <i data-lucide="chevron-right" class="w-4 h-4 text-green-700"></i>
                                            </button>
                                            <button @click="openEmail()" class="flex items-center p-3 bg-blue-50 rounded-lg border border-blue-200 hover:bg-blue-100 transition-colors">
                                                <div class="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center mr-3"><i data-lucide="mail" class="w-5 h-5 text-white"></i></div>
                                                <div class="flex-1 text-left"><p class="font-medium text-blue-900">Send Email</p><p class="text-sm text-blue-700" x-text="requestDetails.email"></p></div>
                                                <i data-lucide="chevron-right" class="w-4 h-4 text-blue-700"></i>
                                            </button>
                                            <button @click="openSms()" class="flex items-center p-3 bg-purple-50 rounded-lg border border-purple-200 hover:bg-purple-100 transition-colors">
                                                <div class="w-10 h-10 bg-purple-600 rounded-lg flex items-center justify-center mr-3"><i data-lucide="message-square" class="w-5 h-5 text-white"></i></div>
                                                <div class="flex-1 text-left"><p class="font-medium text-purple-900">Send SMS</p><p class="text-sm text-purple-700">Text message</p></div>
                                                <i data-lucide="chevron-right" class="w-4 h-4 text-purple-700"></i>
                                            </button>
                                            <template x-if="requestDetails.alt_contact">
                                                <button @click="callCustomer(requestDetails.alt_contact)" class="flex items-center p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                                                    <div class="w-10 h-10 bg-gray-700 rounded-lg flex items-center justify-center mr-3"><i data-lucide="phone" class="w-5 h-5 text-white"></i></div>
                                                    <div class="flex-1 text-left"><p class="font-medium text-gray-900">Call Alt. Number</p><p class="text-sm text-gray-700" x-text="requestDetails.alt_contact"></p></div>
                                ...
                                <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                        <i data-lucide="list-checks" class="w-5 h-5 text-orange-600"></i><span>Status Management</span>
                                    </h4>
                                    <div class="grid grid-cols-1 gap-3">
                                        <template x-if="requestDetails.status==='pending'">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <button @click="confirmAction('Confirm this request? You acknowledge you have the requested quantity available.', () => updateStatus(requestDetails.id,'confirmed'))" class="flex items-center justify-center px-3 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"><i data-lucide="check-circle" class="w-5 h-5 mr-2"></i>Confirm</button>
                                                <button @click="confirmAction('Cancel this request? Customer will be notified and a new Vendor will be assigned.', () => updateStatus(requestDetails.id,'cancelled'))" class="flex items-center justify-center px-3 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"><i data-lucide="x-octagon" class="w-5 h-5 mr-2"></i>Cancel</button>
                                            </div>
                                        </template>
                                        <template x-if="requestDetails.status==='confirmed'">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <button @click="confirmAction('Mark this visit as Completed?', () => updateStatus(requestDetails.id,'completed'))" class="flex items-center justify-center px-3 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><i data-lucide="check-check" class="w-5 h-5 mr-2"></i>Completed</button>
                                                <button @click="confirmAction('Cancel this request? Customer will be notified and a new Vendor will be assigned.', () => updateStatus(requestDetails.id,'cancelled'))" class="flex items-center justify-center px-3 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"><i data-lucide="x-octagon" class="w-5 h-5 mr-2"></i>Cancel</button>
                                            </div>
                                        </template>
                                        <template x-if="requestDetails.status==='completed' || requestDetails.status==='cancelled'">
                                            <div class="p-4 border border-dashed rounded-lg text-sm text-gray-600">No further actions available.</div>
                                        </template>
                                    </div>
                                </div>
                            </div>
    ...
    <div x-cloak x-show="modals.email" x-transition.opacity class="fixed inset-0 z-[60]">
        ...
        <div class="relative w-full max-w-2xl mx-auto my-6 bg-white rounded-xl shadow-lg max-h-[90vh] overflow-hidden">
            <div class="p-4 sm:p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-blue-100 flex items-center justify-between">
                <h3 class="text-lg font-bold text-gray-900">Send Email</h3>
                <button @click="closeEmail()" class="text-gray-500 hover:text-gray-700 p-2"><i data-lucide="x" class="w-5 h-5"></i></button>
            </div>
            <form @submit.prevent="sendEmail()" class="p-4 sm:p-6 space-y-4 overflow-y-auto max-h-[calc(90vh-88px)]">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">To</label>
                    <input type="email" x-model="emailForm.to" readonly class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Subject</label>
                    <input type="text" x-model="emailForm.subject" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white text-gray-900">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Message</label>
                    <textarea rows="6" x-model="emailForm.message" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white text-gray-900"></textarea>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" @click="closeEmail()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Cancel</button>
                    <button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors inline-flex items-center gap-2"><i data-lucide="send" class="w-4 h-4"></i>Send Email</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-cloak x-show="modals.sms" x-transition.opacity class="fixed inset-0 z-[60]">
        ...
        <div class="relative w-full max-w-lg mx-auto top-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg">
            <div class="p-4 sm:p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-blue-100 flex items-center justify-between">
                ...
                <button @click="closeSms()" class="text-gray-500 hover:text-gray-700 p-2"><i data-lucide="x" class="w-5 h-5"></i></button>
            </div>
            <form @submit.prevent="sendSms()" class="p-4 sm:p-6 space-y-4">
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center justify-between">
                    <div><p class="text-sm font-medium text-blue-900">SMS Credits</p><p class="text-xs text-blue-700">Available balance</p></div>
                    <div class="text-right"><p class="text-lg font-bold text-blue-900" x-text="sms.balance"></p><p class="text-xs text-blue-700">credits</p></div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">To</label>
                    <input type="tel" x-model="sms.to" readonly class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Message</label>
                    <textarea rows="4" maxlength="160" x-model="sms.message" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary"></textarea>
                    <div class="text-right text-sm text-gray-500 mt-1"><span x-text="sms.message.length"></span>/160 characters</div>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" @click="closeSms()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Cancel</button>
                    <button type="submit" :disabled="sms.balance<1 || sms.sending" class="px-4 py-2 rounded-lg text-white transition-colors inline-flex items-center gap-2" :class="sms.balance<1 ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary/90'">
                        <i :data-lucide="sms.sending ? 'loader-2' : 'message-square'" :class="sms.sending ? 'animate-spin' : ''" class="w-4 h-4"></i>
                        <span x-text="sms.balance<1 ? 'Insufficient Credits' : (sms.sending ? 'Sending...' : 'Send SMS')"></span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <div x-cloak x-show="modals.confirm" x-transition.opacity class="fixed inset-0 z-[70]">
        ...
        <div class="relative w-full max-w-md mx-auto top-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg">
            <div class="p-5">
                <div class="flex items-start gap-3">
                    <div class="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center"><i data-lucide="alert-triangle" class="w-5 h-5 text-amber-700"></i></div>
                    <div class="flex-1">
                ...
                <div class="flex justify-end gap-3 mt-6">
                    <button @click="closeConfirm()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">No</button>
                    <button @click="runConfirm()" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90">Yes</button>
                </div>
            </div>
        </div>
    </div>

    <div x-cloak x-show="toast.show" x-transition.opacity class="fixed inset-x-0 bottom-4 z-[80] flex justify-center px-4">
        <div class="w-full max-w-md rounded-lg shadow-lg border" :class="toast.type==='success'?'bg-green-50 border-green-200':'bg-red-50 border-red-200'">
            <div class="p-4 flex items-start gap-3">
                <div class="w-9 h-9 rounded-full flex items-center justify-center" :class="toast.type==='success'?'bg-green-100':'bg-red-100'">
                    <i :data-lucide="toast.type==='success' ? 'check' : 'x-circle'" :class="toast.type==='success' ? 'text-green-700' : 'text-red-700'" class="w-5 h-5"></i>
                </div>
                <div class="flex-1">
                    <p class="font-medium" :class="toast.type==='success'?'text-green-900':'text-red-900'" x-text="toast.title"></p>
                    <p class="text-sm mt-0.5" :class="toast.type==='success'?'text-green-800':'text-red-800'" x-text="toast.message"></p>
                </div>
                <button @click="toast.show=false" class="p-2 rounded hover:bg-white/60"><i data-lucide="x" class="w-4 h-4"></i></button>
            </div>
        </div>
    </div>
            ...
            return {
                ...
                get monthGrid() {
                    ...
                    for (let i = 0; i < 42; i++) {
                        ...
                        cells.push({ key: d.toISOString().slice(0,10), day: d.getDate(), inMonth: d.getMonth() === this.calendar.viewMonth, date: d.toISOString().slice(0,10) });
                    }
                    return cells;
                },
                get rangeLabel() {
                ...
                modals: { details: false, email: false, sms: false, confirm: false },
                requestDetails: {},
                ...
                async loadStats(){ try{ const params=new URLSearchParams({action:'getStats', start_date:this.filters.start, end_date:this.filters.end}); const r=await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?`+params.toString()); const j=await r.json(); if(j.success){ this.stats={ pending:parseInt(j.stats.pending||0), confirmed:parseInt(j.stats.confirmed||0), completed:parseInt(j.stats.completed||0), cancelled:parseInt(j.stats.cancelled||0) }; this.nextTickIcons(); } }catch(e){} },
                ...
                async openDetails(id){ this.modals.details=true; this.loadingDetails=true; this.requestDetails={}; this.detailHeader={ title:'Store Visit Request', subtitle:'Loading...' }; try{ const r=await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?action=getRequestDetails&id=${id}`); const j=await r.json(); if(j.success){ this.requestDetails=j.request; this.detailHeader={ title:this.requestDetails.first_name+' '+this.requestDetails.last_name, subtitle:'Visit Request - '+this.requestDetails.product_title }; this.computeStatusPill(); } else { this.toastError('Failed','Failed to load request details'); } } catch(e){ this.toastError('Error','An error occurred while loading details'); } finally{ this.loadingDetails=false; this.nextTickIcons(); } },
                ...
                closeAnyModal(){ if(this.modals.details) this.closeDetails(); if(this.modals.email) this.closeEmail(); if(this.modals.sms) this.closeSms(); if(this.modals.confirm) this.closeConfirm(); if(this.calendar.open) this.closeCalendar(); },
