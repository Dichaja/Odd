...
<div class="min-h-screen bg-gray-50" x-data="storeVisitsPage()" x-init="init()" x-on:keydown.escape="closeAnyModal()">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            <template x-for="cell in monthGrid" :key="cell.key">
                                <button class="w-full aspect-square rounded border text-sm relative"
                                    :class="[cell.inMonth ? 'bg-white hover:bg-gray-50 border-gray-200 text-gray-900' : 'bg-gray-50 text-gray-400 border-gray-200', isInSelectedRange(cell.date) ? 'ring-2 ring-primary/60 ring-offset-1' : '']"
                    ...
                    <div>
                        ...
                        <div class="text-center">
                            <button class="px-4 py-2 bg-primary text-white rounded"
                                @click="selectYear(calendar.viewYear)">Use Year</button>
                        </div>
                    </div>
                </template>
            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <template x-if="requestDetails.status==='confirmed'">
                                                    <button
                                                        ...
                                                        class="text-blue-600 hover:text-blue-800 text-xs p-1 rounded hover:bg-blue-50"><i
                                    ...
                                    <div class="space-y-2 text-sm">
                                        ...
                                        <div class="pt-2">
                                            <a :href="detailProductUrl" target="_blank"
                                                class="text-blue-600 hover:text-blue-800 text-xs underline">View product
                                                details</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-white rounded-lg p-4 border border-gray-200">
                                    <h4 class="font-semibold text-gray-900 mb-3 text-sm">Commission Summary</h4>
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between"><span class="text-gray-600">Commission
                                                Type:</span><span class="font-medium"
                                                x-text="capitalize(requestDetails.commission_type)"></span></div>
                                        <div class="flex justify-between"
                                            x-show="requestDetails.commission_type==='percentage'"><span
                                                class="text-gray-600">Commission
                                                Value:</span><span class="font-medium"
                                                x-text="requestDetails.commission_value+'%'"></span></div>
                                        <div class="flex justify-between"
                                            x-show="requestDetails.commission_type==='flat'"><span
                                                class="text-gray-600">Commission
                                                Value:</span><span class="font-medium"
                                                x-text="'UGX '+formatCurrency(requestDetails.commission_value)+' per unit'"></span>
                                        </div>
                                        <div class="flex justify-between"><span class="text-gray-600">Commission
                                                Amount:</span><span class="font-bold text-gray-900"
                                                x-text="'UGX '+formatCurrency(requestDetails.commission)"></span></div>
                                        <div class="flex justify-between"><span class="text-gray-600">Wallet
                                                Balance:</span><span class="font-medium"
                                                x-text="'UGX '+formatCurrency(requestDetails.wallet_balance)"></span>
                                        </div>
                                        <div class="flex justify-between"><span class="text-gray-600">Projected
                                                Balance:</span><span class="font-medium"
                                                :class="requestDetails.projected_balance<0?'text-red-600':'text-gray-900'"
                                                x-text="'UGX '+formatCurrency(requestDetails.projected_balance)"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="space-y-6">
                                ...
                                <div class="space-y-4">
                                    <div class="relative">
                                        <img :src="detailProductImage" alt="Product Image"
                                            ...
                                            @error="detailProductImage = placeholderImg(requestDetails.product_title)">
                                        <a :href="detailProductUrl" target="_blank"
                                    ...
                                    <div>
                                        <a :href="detailProductUrl" target="_blank"
                                            class="font-semibold text-gray-900 text-lg underline decoration-dotted underline-offset-4 hover:text-blue-600 transition">
                            ...
                            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                    <i data-lucide="calendar-check" class="w-5 h-5 text-green-600"></i><span>Visit
                                        Details</span>
                                </h4>
                                <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2 mt-1">
                                                ...
                                                <template x-if="requestDetails.status==='confirmed'">
                                                    <button
                                                        @click="openRescheduleCalendar(requestDetails.id, requestDetails.visit_date)"
                                                        class="text-blue-600 hover:text-blue-800 text-sm p-1 rounded hover:bg-blue-50"><i
                                                            data-lucide="edit-3" class="w-4 h-4"></i></button>
                                                </template>
                                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                        <i data-lucide="phone"
                                            class="w-5 h-5 text-purple-600"></i><span>Communication</span>
                                    </h4>
                                    <template x-if="requestDetails.status!=='confirmed'">
                                        <div
                                            class="p-4 border border-dashed border-gray-300 rounded-lg text-sm text-gray-600">
                                            Confirm this request to view customer communication options.</div>
                                    </template>
                                    <template x-if="requestDetails.status==='confirmed'">
                                ...
                                <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-3">
                                        <i data-lucide="list-checks" class="w-5 h-5 text-orange-600"></i><span>Status
                                            Management</span>
                                    </h4>
                                    <div class="grid grid-cols-1 gap-3">
                                        <template x-if="requestDetails.status==='pending'">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <button
                                                    ...
                                                    class="flex items-center justify-center px-3 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"><i
                                                        data-lucide="check-circle"
                                                        class="w-5 h-5 mr-2"></i>Confirm</button>
                                                <button
                                                    ...
                                                    class="flex items-center justify-center px-3 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"><i
                                                        data-lucide="x-octagon" class="w-5 h-5 mr-2"></i>Cancel</button>
                                            </div>
                                        </template>
                                        <template x-if="requestDetails.status==='confirmed'">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <button @click="openCommission(requestDetails)"
                                                    class="flex items-center justify-center px-3 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><i
                                                        data-lucide="check-check"
                                                        class="w-5 h-5 mr-2"></i>Completed</button>
                                                <button
                                                    @click="confirmAction('Cancel this request? Customer will be notified and a new Vendor will be assigned.', () => updateStatus(requestDetails.id,'cancelled'))"
                                                    class="flex items-center justify-center px-3 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"><i
                                                        data-lucide="x-octagon" class="w-5 h-5 mr-2"></i>Cancel</button>
    ...
    <div x-cloak x-show="modals.commission" x-transition.opacity class="fixed inset-0 z-[70]">
        <div class="absolute inset-0 bg-black/60" @click="closeCommission()"></div>
        <div class="relative w-full max-w-xl mx-auto top-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg">
            <div class="p-5 border-b bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900">Charge Commission & Complete</h3>
                <button @click="closeCommission()" class="p-2 rounded hover:bg-white/50"><i data-lucide="x"
                        class="w-5 h-5"></i></button>
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="border rounded-lg p-3 bg-gray-50">
                        ...
                        <p class="text-lg font-semibold text-gray-900"
                            x-text="'UGX '+formatCurrency(commission.subtotal)"></p>
                    </div>
                    <div class="border rounded-lg p-3 bg-gray-50">
                        <p class="text-xs text-gray-600">Commission</p>
                        <p class="text-lg font-semibold text-gray-900"
                            x-text="'UGX '+formatCurrency(commission.amount)"></p>
                    </div>
                    <div class="border rounded-lg p-3 bg-gray-50">
                        <p class="text-xs text-gray-600">Wallet Balance</p>
                        <p class="text-lg font-semibold text-gray-900"
                            x-text="'UGX '+formatCurrency(commission.wallet_balance)"></p>
                    </div>
                    <div class="border rounded-lg p-3"
                        :class="commission.projected_balance<0?'bg-red-50 border-red-200':'bg-gray-50'">
                        <p class="text-xs" :class="commission.projected_balance<0?'text-red-700':'text-gray-600'">
                            Projected Balance</p>
                        <p class="text-lg font-semibold"
                            :class="commission.projected_balance<0?'text-red-800':'text-gray-900'"
                            x-text="'UGX '+formatCurrency(commission.projected_balance)"></p>
                    </div>
                </div>
                <div class="text-sm text-gray-700">
                    <p
                        x-text="'Type: '+capitalize(commission.type)+(commission.type==='percentage' ? ' ('+commission.value+'%)' : ' (UGX '+formatCurrency(commission.value)+' per unit)')">
                    </p>
                    <p class="mt-1 text-gray-600">Your wallet may go into negative. The commission will be collected
                        immediately and the request will be marked as completed.</p>
                </div>
            </div>
            <div class="p-5 border-t flex items-center justify-end gap-3">
                <button @click="closeCommission()"
                    class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</button>
                <button @click="confirmCommissionCharge()" :disabled="commission.charging"
                    class="px-4 py-2 rounded-lg text-white inline-flex items-center gap-2"
                    :class="commission.charging ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'">
                    <i :data-lucide="commission.charging ? 'loader-2' : 'badge-check'"
                        :class="commission.charging ? 'animate-spin' : ''" class="w-4 h-4"></i>
                    <span x-text="commission.charging ? 'Processing...' : 'Charge & Complete'"></span>
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        function storeVisitsPage() {
            return {
                ...
                modals: { details: false, email: false, sms: false, confirm: false, commission: false },
                requestDetails: {},
                ...
                commission: { request_id: null, subtotal: 0, type: '', value: 0, amount: 0, wallet_balance: 0, projected_balance: 0, charging: false },
                detailProductUrl: '#',
                detailProductImage: 'https://placehold.co/800x480/e2e8f0/1e293b?text=Product',
                ...
                formatCurrency(n) { const num = parseFloat(n || 0); return (new Intl.NumberFormat('en-UG', { minimumFractionDigits: 0, maximumFractionDigits: 2 })).format(isNaN(num) ? 0 : num) },
                ...
                async openDetails(id) { this.modals.details = true; this.loadingDetails = true; this.requestDetails = {}; this.detailHeader = { title: 'Store Visit Request', subtitle: 'Loading...' }; this.detailProductUrl = '#'; this.detailProductImage = this.placeholderImg('Product'); try { const r = await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?action=getRequestDetails&id=${id}`); const j = await r.json(); if (j.success) { this.requestDetails = j.request; this.detailHeader = { title: this.requestDetails.first_name + ' ' + this.requestDetails.last_name, subtitle: 'Visit Request - ' + this.requestDetails.product_title }; const pid = this.requestDetails.product_id || this.requestDetails.productId || this.requestDetails.pid; this.detailProductUrl = pid ? this.productDetailsUrl(pid) : '#'; this.detailProductImage = await this.resolveProductImage(pid, this.requestDetails.product_title); this.computeStatusPill() } else { this.toastError('Failed', 'Failed to load request details') } } catch (e) { this.toastError('Error', 'An error occurred while loading details') } finally { this.loadingDetails = false; this.nextTickIcons() } },
                ...
                async updateStatus(id, status) { if (status === 'completed') { const r = await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?action=getRequestDetails&id=${id}`); const j = await r.json(); if (j.success) { this.openCommission(j.request) } else { this.toastError('Failed', j.message || 'Unable to prepare completion') } return } try { const fd = new FormData(); fd.append('request_id', id); fd.append('status', status); const r = await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?action=updateRequestStatus`, { method: 'POST', body: fd }); const j = await r.json(); if (j.success) { this.toastSuccess('Success', 'Status updated successfully'); await this.openDetails(id); this.loadRequests(); this.loadStats() } else { this.toastError('Failed', j.message || 'Failed to update status') } } catch (e) { this.toastError('Error', 'An error occurred while updating status') } },
                openCommission(req) { this.commission.request_id = req.id; this.commission.subtotal = parseFloat(req.total_value || 0); this.commission.type = (req.commission_type || '').toLowerCase(); this.commission.value = parseFloat(req.commission_value || 0); this.commission.amount = parseFloat(req.commission || 0); this.commission.wallet_balance = parseFloat(req.wallet_balance || 0); this.commission.projected_balance = parseFloat((req.projected_balance ?? (this.commission.wallet_balance - this.commission.amount))); this.commission.charging = false; this.modals.commission = true; this.nextTickIcons() },
                closeCommission() { this.modals.commission = false },
                async confirmCommissionCharge() { if (!this.commission.request_id) return; this.commission.charging = true; try { const fd = new FormData(); fd.append('request_id', this.commission.request_id); const r = await fetch(`${window.BASE_URL}vendor-store/fetch/manageBuyInStore.php?action=completeRequest`, { method: 'POST', body: fd }); const j = await r.json(); if (j.success) { this.toastSuccess('Success', 'Commission charged and request completed'); this.modals.commission = false; await this.openDetails(this.commission.request_id); this.loadRequests(); this.loadStats() } else { this.toastError('Failed', j.message || 'Failed to complete request') } } catch (e) { this.toastError('Error', 'An error occurred while completing request') } finally { this.commission.charging = false; this.nextTickIcons() } },
                callCustomer(phone) { window.location.href = `tel:${phone}` },
                async openSms() { if ((this.requestDetails.status || '') !== 'confirmed') return; await this.fetchSmsBalance(); this.sms.to = this.requestDetails.phone; this.sms.message = `Hello ${this.requestDetails.first_name}, regarding your Buy in Store request for ${this.requestDetails.product_title}.`; this.modals.sms = true; this.nextTickIcons() },
                ...
                closeAnyModal() { if (this.modals.details) this.closeDetails(); if (this.modals.email) this.closeEmail(); if (this.modals.sms) this.closeSms(); if (this.modals.confirm) this.closeConfirm(); if (this.modals.commission) this.closeCommission(); if (this.calendar.open) this.closeCalendar() },
