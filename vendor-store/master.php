...
/* ------------------------------------------------------------------
   3.  FETCH STORE INFO  (name & owner)  +  ROLE RESOLUTION
-------------------------------------------------------------------*/
...
if (!$store) {
    // invalid / deleted store id in session
    header('Location: ' . BASE_URL . 'account/dashboard');
    exit;
}
...
/* — Determine user’s role for this store — */
...
if (!$isAdmin && !$isOwner) {
    ...
    $isManager = (bool) $mgrStmt->fetchColumn();
}
...
/* ------------------------------------------------------------------
   5.  PAGE TITLE + USER/ST ORE  DATA FOR HEADER
-------------------------------------------------------------------*/
...
<body class="bg-user-content font-rubik">
            ...
            <header class="user-header sticky top-0 z-40 border-b border-gray-100">
                <div class="flex h-16 items-center justify-between px-6">
                    <div class="flex items-center gap-4">
                        ...
                        <h1 class="text-xl font-semibold text-secondary truncate max-w-full whitespace-nowrap overflow-hidden"
                            title="<?= htmlspecialchars($pageTitle ?? 'Vendor Dashboard') ?>">
                            <?= htmlspecialchars($pageTitle ?? 'Vendor Dashboard') ?>
                        </h1>
                    </div>
                        ...
                        <div class="relative" id="userDropdown">
                            ...
                            <div id="userDropdownMenu"
                                ...
                                <div class="px-4 py-3 bg-gray-50">
                                    ...
                                    <p class="text-xs text-gray-500"><?= htmlspecialchars($storeName) ?></p> <p class="text-xs text-gray-500 mt-1">Role: <?= $userRole ?></p>
                                </div>
                                <a href="<?= BASE_URL ?>vendor-store/store-profile"
                                    ...
                                    <i class="fas fa-store w-5 h-5 text-gray-400"></i>
                                    Store Profile
                                </a>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const activeItem = document.querySelector('.user-nav-item.active');
            if (activeItem) {
                const sidebarNav = document.getElementById('sidebarNav');
                sidebarNav.scrollTo({ top: activeItem.offsetTop - sidebarNav.offsetTop - 20, behavior: 'smooth' });
            }
        });
