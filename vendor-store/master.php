...
/* ------------------------------------------------------------------
   2.  STORE SESSION  – must have an active store in session
-------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------
   3.  FETCH STORE INFO + ROLE
-------------------------------------------------------------------*/
$storeStmt = $pdo->prepare(
    "SELECT id,name,owner_id FROM vendor_stores
     WHERE id=:sid AND status IN ('active','pending','inactive','suspended')"
);
...
/* ------------------------------------------------------------------
   5.  STORE SESSION ID FOR NOTIFICATIONS
-------------------------------------------------------------------*/
...
$storeInitials = '';
$parts = explode(' ', $storeName);
for ($i = 0; $i < min(2, count($parts)); $i++) {
    if ($parts[$i] !== '')
        $storeInitials .= strtoupper($parts[$i][0]);
}
...
<head>
    ...
    <script>
        ...
        tailwind.config = {
            theme: { extend: { colors: { primary: '#D92B13', secondary: '#1a1a1a', 'gray-text': '#4B5563', 'user-primary': '#D92B13', 'user-secondary': '#F8C2BC', 'user-accent': '#E6F2FF', 'user-content': '#F5F9FF' }, fontFamily: { rubik: ['Rubik', 'sans-serif'] } } }
        }
    </script>
    <style>
        body {
...
<body class="bg-user-content font-rubik">
                    ...
                    <?php foreach ($menuItems as $category): ?>
                        ...
                        <div class="space-y-1 mb-2">
                            <?php foreach ($category['items'] as $key => $item): ?>
                                <a href="<?= BASE_URL ?>vendor-store/<?= $key ?>" id="nav-<?= $key ?>"
    ...
    <script>
        /* sidebar */
        const sidebar = document.getElementById('sidebar'), sidebarToggle = document.getElementById('sidebarToggle'), overlay = Object.assign(document.createElement('div'), { className: 'fixed inset-0 bg-black/20 z-40 lg:hidden hidden' }); document.body.appendChild(overlay);
        sidebarToggle.addEventListener('click', toggleSidebar); overlay.addEventListener('click', toggleSidebar);
        function toggleSidebar() { const open = !sidebar.classList.contains('-translate-x-full'); sidebar.classList.toggle('-translate-x-full', open); overlay.classList.toggle('hidden', open); document.body.classList.toggle('overflow-hidden', !open); if (!open) { sidebar.classList.add('animate-slide-in'); setTimeout(() => sidebar.classList.remove('animate-slide-in'), 300); } }

        /* dropdown */
        const userDropdown = document.getElementById('userDropdown'), userDropdownMenu = document.getElementById('userDropdownMenu');
        ...
        /* nav scroll */
        document.addEventListener('DOMContentLoaded', () => { const active = document.querySelector('.user-nav-item.active'); const nav = document.getElementById('sidebarNav'); if (active && nav) { nav.scrollTo({ top: active.offsetTop - nav.offsetTop - 20, behavior: 'smooth' }); } });

        /* logout */
        function logoutUser() { $.ajax({ url: BASE_URL + 'auth/logout', type: 'POST', contentType: 'application/json', dataType: 'json', success: d => d.success ? location.href = BASE_URL : alert(d.message || 'Failed to logout'), error: () => alert('Failed to connect.') }); }

        /* notifications */
        function notifComponent() {
            return {
                open: false, notes: [], count: 0,
                toggle() { this.open = !this.open; if (this.open) this.load(); },
                load() {
                    fetch(BASE_URL + 'fetch/manageNotifications.php?action=fetch&limit=50')
                        .then(r => r.json()).then(res => { this.notes = res.data || []; this.count = this.notes.filter(n => n.is_seen == 0).length; });
                },
                markSeenReq(id) {
                    fetch(BASE_URL + 'fetch/manageNotifications.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ action: 'markSeen', target_id: id }) });
                },
                handleClick(note) {
                ...
                dismiss(id) {
                    fetch(BASE_URL + 'fetch/manageNotifications.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ action: 'dismiss', target_id: id }) }).then(() => this.load());
                },
                formatDate(ts) {
                    const d = new Date(ts.replace(' ', 'T')), now = new Date(), diff = (now - d) / 1000, today = new Date(now.getFullYear(), now.getMonth(), now.getDate()), yest = new Date(today); yest.setDate(today.getDate() - 1);
                    const time = d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }); if (diff < 60) return 'Now'; if (d >= today) return 'Today ' + time; if (d >= yest && d < today) return 'Yesterday ' + time; return d.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
                },
