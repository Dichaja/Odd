...
function checkEmailAvailability(PDO $pdo, string $email, string $storeId, ?string $currentUserId)
{
    ...
    try {
        ...
        if (!$user) {
            echo json_encode(['success' => false, 'error' => 'User with this email does not exist', 'code' => 'user_not_found']);
            return;
        }
        if ($user['status'] !== 'active') {
            echo json_encode(['success' => false, 'error' => 'User account is not active', 'code' => 'user_inactive']);
            return;
        }
        ...
        if ($ownerStmt->fetchColumn()) {
            echo json_encode(['success' => false, 'error' => 'This user is the store owner', 'code' => 'is_owner']);
            return;
        }
        ...
        if ($managerStatus) {
            echo json_encode(['success' => false, 'error' => 'This user is already a manager for this store', 'code' => 'already_manager', 'status' => $managerStatus]);
            return;
        }
...
function inviteManager(PDO $pdo, string $currentUser)
{
    ...
    try {
        ...
        if ($ownerStmt->fetchColumn()) {
            $pdo->rollBack();
            echo json_encode(['success' => false, 'error' => 'Cannot add store owner as a manager', 'code' => 'is_owner']);
            return;
        }
        ...
        if ($existingManager) {
            if ($existingManager['status'] === 'active') {
                $pdo->rollBack();
                echo json_encode(['success' => false, 'error' => 'This user is already an active manager for this store', 'code' => 'already_active']);
                return;
            } else {
                $updateStmt = $pdo->prepare("UPDATE store_managers SET status = 'inactive', role = ?, approved = 0, updated_at = NOW() WHERE id = ?");
            ...
            if ($removedManager && !$reinvite) {
                ...
                echo json_encode([
                    ...
                    'user_info' => ['first_name' => $user['first_name'], 'last_name' => $user['last_name']]
                ]);
                return;
            }
            if ($removedManager && $reinvite) {
                $updateStmt = $pdo->prepare("UPDATE store_managers SET status = 'inactive', role = ?, approved = 0, updated_at = NOW(), created_at = NOW() WHERE id = ?");
                $updateStmt->execute([$role, $removedManager['id']]);
                $managerId = $removedManager['id'];
            } else {
                $managerId = generateUlid();
                $insertStmt = $pdo->prepare("INSERT INTO store_managers (id, store_id, user_id, role, status, added_by, approved, created_at, updated_at) VALUES (?, ?, ?, ?, 'inactive', ?, 0, NOW(), NOW())");
                $insertStmt->execute([$managerId, $storeId, $userId, $role, $currentUser]);
            }
        }
        $emailSent = sendManagerInvitationEmail($email, $user['first_name'], $user['last_name'], $storeName, $role);
        $pdo->commit();
        echo json_encode(['success' => true, 'message' => 'Manager invitation sent successfully', 'manager_id' => $managerId, 'email_sent' => $emailSent]);
    } catch (Exception $e) {
...
function updateManagerStatus(PDO $pdo, string $currentUser)
{
    ...
    try {
        ...
        $checkStmt = $pdo->prepare("
            SELECT sm.id, sm.status, sm.user_id, sm.approved, u.first_name, u.last_name, u.email 
            FROM store_managers sm
            JOIN zzimba_users u ON sm.user_id = u.id
        ...
        if ((int) $manager['approved'] !== 1) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'Cannot change status before approval']);
            return;
        }
        if ($manager['status'] === $newStatus) {
            echo json_encode(['success' => true, 'message' => 'Manager status is already ' . $newStatus, 'manager' => ['id' => $managerId, 'status' => $newStatus, 'name' => $manager['first_name'] . ' ' . $manager['last_name']]]);
            return;
        }
        $updateStmt = $pdo->prepare("UPDATE store_managers SET status = ?, updated_at = NOW() WHERE id = ?");
        $updateStmt->execute([$newStatus, $managerId]);
        $emailSent = sendManagerStatusChangeEmail($manager['email'], $manager['first_name'], $manager['last_name'], $storeName, $newStatus);
        echo json_encode(['success' => true, 'message' => 'Manager status updated successfully', 'manager' => ['id' => $managerId, 'status' => $newStatus, 'name' => $manager['first_name'] . ' ' . $manager['last_name']], 'email_sent' => $emailSent]);
    } catch (Exception $e) {
...
function updateManagerRole(PDO $pdo, string $currentUser)
{
    ...
    try {
        ...
        if ($manager['role'] === $newRole) {
            echo json_encode(['success' => true, 'message' => 'Manager role is already ' . $newRole, 'manager' => ['id' => $managerId, 'role' => $newRole, 'name' => $manager['first_name'] . ' ' . $manager['last_name']]]);
            return;
        }
        $updateStmt = $pdo->prepare("UPDATE store_managers SET role = ?, updated_at = NOW() WHERE id = ?");
        $updateStmt->execute([$newRole, $managerId]);
        $emailSent = sendManagerRoleChangeEmail($manager['email'], $manager['first_name'], $manager['last_name'], $storeName, $newRole);
        echo json_encode(['success' => true, 'message' => 'Manager role updated successfully', 'manager' => ['id' => $managerId, 'role' => $newRole, 'name' => $manager['first_name'] . ' ' . $manager['last_name']], 'email_sent' => $emailSent]);
    } catch (Exception $e) {
...
function removeManager(PDO $pdo, string $currentUser)
{
    ...
    try {
        ...
        $updateStmt = $pdo->prepare("UPDATE store_managers SET status = 'removed', updated_at = NOW() WHERE id = ?");
        $updateStmt->execute([$managerId]);
        $emailSent = sendManagerRemovalEmail($manager['email'], $manager['first_name'], $manager['last_name'], $storeName);
        echo json_encode(['success' => true, 'message' => 'Manager removed successfully', 'manager' => ['id' => $managerId, 'name' => $manager['first_name'] . ' ' . $manager['last_name']], 'email_sent' => $emailSent]);
    } catch (Exception $e) {
