...
final class EntityLookup
{
    ...
    private function vendorName(string $id): string
    {
...
final class SettingsRepo
{
    ...
    {
        $in = implode("','", array_map('strval', $applicable));
        $sql = "SELECT id,setting_key,setting_name,setting_value,setting_type,applicable_to FROM zzimba_credit_settings WHERE setting_key=:k AND category=:c AND status='active' AND applicable_to IN ('all','$in') ORDER BY CASE WHEN applicable_to='all' THEN 1 ELSE 2 END LIMIT 1";
        $st = $this->pdo->prepare($sql);
        $st->execute([':k' => $key, ':c' => $category]);
        return $st->fetch(\PDO::FETCH_ASSOC) ?: null;
    }
}
...
final class CreditService
{
    ...
    public static function makeMobileMoneyPayment(array $opts): array
    {
        ...
        $rec = self::$bus->recipients(
            ...
            !empty($opts['user_id']) ? ['id' => $opts['user_id'], 'message' => "You initiated a Mobile Money top-up of {$amount} UGX from {$msisdn}. Transaction ID: {$reference}"] : null,
            !empty($opts['vendor_id']) ? ['id' => $opts['vendor_id'], 'message' => "Mobile Money top-up of {$amount} UGX was initiated successfully. Transaction ID: {$reference}"] : null
        );
    ...
    public static function checkRequestStatus(string $internalRef): array
    {
        ...
        if ($previousStatus !== $newStatus) {
            ...
            if ($txn) {
                ...
                $adminMessage = ['message' => "{$entity['type']} {$entity['name']}'s transaction {$txn['transaction_id']} status changed from {$previousStatus} to {$newStatus}. Amount: {$txn['amount_total']} UGX"];
                $userMessage = null;
                $vendorMessage = null;
                if ($newStatus === 'SUCCESS') {
                    if (!empty($txn['user_id']))
                        $userMessage = ['id' => $txn['user_id'], 'message' => "Payment successful! {$txn['amount_total']} UGX has been credited to your wallet. (Transaction ID: {$txn['transaction_id']})"];
                    if (!empty($txn['vendor_id']))
                        $vendorMessage = ['id' => $txn['vendor_id'], 'message' => "Payment successful! {$txn['amount_total']} UGX has been credited to your store wallet. (Transaction ID: {$txn['transaction_id']})"];
                } elseif ($newStatus === 'FAILED') {
                    if (!empty($txn['user_id']))
                        $userMessage = ['id' => $txn['user_id'], 'message' => "Payment failed for Transaction {$txn['transaction_id']}. Amount: {$txn['amount_total']} UGX"];
                    if (!empty($txn['vendor_id']))
                        $vendorMessage = ['id' => $txn['vendor_id'], 'message' => "Payment failed for Transaction {$txn['transaction_id']}. Amount: {$txn['amount_total']} UGX"];
                }
    ...
    {
        self::boot();
        $wallet = $ownerType === 'PLATFORM' ? self::$pdo->query("SELECT wallet_id,wallet_number,wallet_name,current_balance,status,created_at FROM zzimba_wallets WHERE owner_type='PLATFORM' AND status='active' LIMIT 1")->fetch(\PDO::FETCH_ASSOC)
    ...
    public static function logCashTopup(array $opts): array
    {
        ...
        try {
            ...
            if (!empty($adminPhones)) {
                $message = sprintf("%s initiated a cash top-up of %s UGX. Txn ID: %s. Login to confirm.", $initiator, number_format($amount, 2), $txnId);
