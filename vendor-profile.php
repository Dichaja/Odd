...
<div id="loading-state" class="flex flex-col items-center justify-center py-12">
    <div class="border-4 border-gray-200 border-l-primary rounded-full w-10 h-10 animate-spin mb-4"></div>
...
<div id="not-found-state"
    ...
    <a href="<?= BASE_URL ?>"
        class="inline-block bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary/90 transition-colors">
        Return to Home
    </a>
</div>
                        ...
                        <h1 id="vendor-name-container" class="text-3xl font-bold text-secondary">
                            ...
                            <?php if ($canEdit): ?>
                                <i id="edit-name-button"
                                    class="fas fa-pen ml-2 text-gray-500 hover:text-primary text-sm cursor-pointer transition-colors"></i>
                            <?php endif; ?>
                        </h1>
                        ...
                        <div id="location-section">
                            ...
                            <?php else: ?>
                                <button id="view-location-btn"
                                    class="flex items-center text-primary text-sm font-medium hover:underline transition-all view-btn"
            ...
            <div class="ml-0 sm:ml-auto flex items-center gap-2">
                ...
                <div class="flex gap-2">
                    <button onclick="copyLink()"
                        class="flex items-center justify-center w-6 h-6 rounded-full text-primary border-[1.5px] border-primary bg-transparent hover:bg-red-50 hover:-translate-y-0.5 transition-all relative">
        ...
        <div class="mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                <h2 class="text-2xl font-bold text-gray-800">
                    <i class="fa-solid fa-box-open mr-2 text-primary"></i>
                    Products
                </h2>
...
<div id="buyInStoreModal" class="fixed inset-0 z-50 hidden">
    ...
    <div
        ...
        <div
            ...
            <div>
                <h3 class="text-xl font-bold text-gray-900" id="buyInStoreTitle">Complete Your Request</h3>
            </div>
                ...
                <div class="w-full md:w-1/2 border-r border-gray-100 overflow-y-auto">
                    ...
                    <form id="buyInStoreForm" class="p-6 space-y-6 hidden">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Please select a date when you plan to visit our store
                            </p>
                        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg w-11/12 max-w-md mx-auto my-8 overflow-y-auto max-h-screen p-5">
            ...
            <div class="flex justify-end gap-2 pt-2 border-t border-gray-200">
                ...
                <button type="button" id="crop-button"
                    class="hidden px-4 py-2 bg-primary text-white rounded-md hover:bg-primary/90"
...
<script>
    ...
    function populateCategoryFilter(categories) {
        ...
        const activeCatsWithProducts = categories.filter(cat =>
            cat.status === 'active' && cat.product_count > 0
        );
    ...
    async function renderProductsForDisplay(products, clearExisting = true) {
        ...
        for (const product of products) {
            ...
            if (!isLoggedIn && !isAdmin && !isOwnerOrManager) {
...
<div class="view-more-prices view-btn">View More Prices</div>`;
            ...
            if (!isAdmin && !isOwnerOrManager) {
                if (isLoggedIn) {
                    actionButtons = `
                        <button onclick="buyInStore('${product.store_product_id}')" class="bg-primary hover:bg-primary/90 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center flex-1 justify-center text-xs md:text-sm">
                    ...
                    actionButtons = `
                        <button onclick="openAuthModal(); return false;" class="bg-primary hover:bg-primary/90 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center flex-1 justify-center text-xs md:text-sm">
                            <i class="fas fa-shopping-cart mr-1"></i>Buy in Store
                        </button>
                        <button onclick="openVendorSellModal('${product.id}', '${escapeHtml(product.name)}')" class="bg-sky-600 hover:bg-sky-700 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center flex-1 justify-center text-xs md:text-sm">
                            <i class="fas fa-tag mr-1"></i>Sell
            ...
            <div class="relative group">
                ...
                <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                    <a href="${BASE_URL}view/product/${product.id}" target="_blank" class="bg-white text-gray-800 px-4 py-2 rounded-lg font-medium hover:bg-primary hover:text-white transition-colors text-sm">View Details</a>
                </div>
            </div>
    ...
    async function buyInStore(productId) {
        if (isAdmin) {
            showToast("Administrators cannot place store orders.", "error");
            return;
        }
